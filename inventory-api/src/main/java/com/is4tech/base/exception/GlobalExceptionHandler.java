...
public class GlobalExceptionHandler {
    ...
    public ProblemDetail handleCustomExceptions(Exceptions exception) {
        ProblemDetail errorDetail = null;
        ...
        if (exception.getMessage().contains("email")) {
            ...
            errorDetail.setProperty("description", "The email is already registered");
        }

        if (exception.getMessage().contains("phone")){
            errorDetail = ProblemDetail.forStatusAndDetail(HttpStatusCode.valueOf(409), exception.getMessage());
            errorDetail.setProperty("description", "The phone number is already registered");
        }

        if (exception.getMessage().contains("username")){
            errorDetail = ProblemDetail.forStatusAndDetail(HttpStatusCode.valueOf(409), exception.getMessage());
            errorDetail.setProperty("descrption", "The username is already registered");
        }
        ...
        errorDetail.setProperty("description", "One or more required fields are empty");
        return errorDetail;
    }
